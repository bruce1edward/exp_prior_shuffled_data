function [Pi_x,order,count] = mcmc_covariance(x, y, order, mcmc_steps, burn_steps, omega, theta)
%{
...
%}
    ...
    for m_step = 1: mcmc_steps
        %sum(order ~=1:n)
                ...
                p21 = trace(n*omega*S_pi)/2 + theta*(sum(order ~= 1:n) - sum(order_ ~= 1:n)); % compute the acceptance raito
                %track(m_step) = p21;
