function [] = experiment_EM_covariance_wine(which)
rng(which)%which = 1;
k_n = [0.2, 0.4, 0.6];
...
data = importdata('..\data\wine\wine.data');
data(:,1) = [];x = data(:,1:7);
p = 7;y = data(:,8:13);
q = 6;[n,d] = size(data);
...
theta = [0 1./(10:-1:1)*Choose_theta(n,K) Choose_theta(n,K):0.8:Choose_theta(n,K)+5];
...
save(['C:\Users\zwang39\OneDrive - George Mason University\Paper(EM)\code\nips2021\functions\diag\result1\results_experiment_' num2str(which) '.mat']);
...
export_fig(['diag/plot1/results_experiment_' num2str(which) '.pdf'])
