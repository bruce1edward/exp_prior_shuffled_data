...
p6 = errorbar(k_n, MSE_beta(:,6), 3*std_beta(:,6)/sqrt(iters), 'c-x', 'LineWidth', 1.5, 'MarkerSize',9, 'MarkerFaceColor', 'r');
xlabel('k*B/n', 'FontSize', 14)
%ylabel('Relative Estiamtion error', 'FontSize', 14)
yticks([0 : 0.2 :1.6])
ylim([min(MSE_beta,[],'all') 1.6])
xlim([min(k_n) max(k_n)])
xticks(k_n)
lh = legend([p1 p2 p3 p4 p5 p6], {'naive', 'oracle','EM','EMB','LL','C'}, 'location', [0.3416 0.6982 0.3804 0.1833]);
lh.NumColumns = 2;
lh.FontSize = 14;
